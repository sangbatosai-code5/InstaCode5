...
class XORGID:
	...
	def cariFollowernya(self):
		...
		sleep(2)
		popup = mybrowser.find_element_by_class_name('isgrP')
		
		followers_array = []
		while len(followers_array) <= jml_follA:
			i=1
			mybrowser.execute_script('arguments[0].scrollTop = arguments[0].scrollHeight', popup)
			sleep(2)
			followers = mybrowser.find_elements_by_class_name('FPmhX')
