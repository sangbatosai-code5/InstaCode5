...
class XORGTAG:
	...
	def cekcommentator(self):
		mybrowser=self.mybrowser
		mybrowser.find_element_by_xpath("//button[contains(@class, 'sqdOP yWX7d     _8A5w5    ')]").click()
		sleep(2)
		self.popupA = mybrowser.find_element_by_xpath("/html/body/div[5]/div/div/div[2]")
		popupA = self.popupA
		self.locNamaProfile = popupA.find_element_by_xpath("//h3[contains(@class,'_6lAjh ')]")
		...
		linkNama = self.popupA.find_elements_by_tag_name('h3')
		
		self.nama = [name.text for name in linkNama if name.text != '']
		nama = self.nama
		popupA = self.popupA
		locNamaProfile = self.locNamaProfile
		print(Blue+"Terdapat komentar dari:")
		print(nama, Reset)


	def daftarCommentator(self):
		mybrowser=self.mybrowser
		hashtagnya=self.hashtagnya
		...
		try:
			...
			while i <= viewernya:
				...
				if(len(mybrowser.find_elements_by_xpath("//h3[contains(@class,'_6lAjh ')]")) > 0):
					self.cekcommentator()
				if(len(mybrowser.find_elements_by_xpath("//h3[contains(@class,'_6lAjh ')]")) < 0):
					print(Red+"TIDAK ADA YANG KOMENT !!!"+Reset)
					continue
				sleep(2)
				self.nama
				for yangCommnya in self.nama:
					arrayComm.append(yangCommnya)
				sleep(2)
